...
var source3 = new VideoSource(renderer, { src: "https://streamable.com/fkfqth" } );
...
filemanager.load_set("/yt_set/vj")

var filemanager2 = new FileManager( source2 )
filemanager2.load_set("/yt_set/videosynthesis")
...
setInterval(function() {
  if ( audioanalysis1.render() > 0.99 && !wasSet ) {
    ...
    if (beats%16 == 0 && dice < 0.64 )  {
      filemanager.changez(); 
      setTimeout(function() { source1.jump() }, 1500 ) 
    }
    if (beats%12 == 0 && dice < 0.64 ) {
...
document.getElementById('search1').onclick = function() {
 filemanager.load_set("/yt_set/" + document.getElementById('input1').value )
 console.log("loaded set: ", filemanager.set)
}
...
document.getElementById('search2').onclick = function() {
 filemanager2.load_set("/yt_set/" + document.getElementById('input2').value )
 console.log("loaded set: ", filemanager2.set)
}
